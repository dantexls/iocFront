...
function movediv(direction, act) {
	var finalval = null;
	var cssvalue = $("#replace").css(direction);
	...
	$("#replace").css(direction, finalval + "px");
	getcssvalue();
	if (magnetstatus == 1) {
		checkval();
	}
 ...
 * var finalval = null; var cssvalue = $("#replace").css(direction); var action =
 ...
 * $("#replace").css(direction, finalval + "px"); getcssvalue(); if (magnetstatus ==
 * 1) { checkval(); } getcssvalue(); getchannelarea(); getrowcolumn(); }
 */

function getcssvalue() {
	var top = $("#replace").css("top");
	var height = $("#replace").css("height");
	var width = $("#replace").css("width");
	var left = $("#replace").css("left");
	$("#distop").val(top);
...
function getchannelarea() {
	var areax = 0;
	var areay = 0;
	var top = parseInt($("#replace").css("top"));
	var left = parseInt($("#replace").css("left"));
	if (left < 220 + 192) {
		areax = 1;
	...
	$("#replace").attr("area", areay + "_" + areax);
}

function getrowcolumn() {
	var row = 0;
	var column = 0;
	var height = parseInt($("#replace").css("height"));
	var width = parseInt($("#replace").css("width"));
	if (height / 108 <= 1) {
		row = 1;
	} else {
	...
	$("#replace").attr("column", column);
	$("#replace").attr("row", row);
}
...
function checkval() {
	...
	if (heightval > 540) {
		heightval = 540;
		$("#replace .dialogbody").css("height", "540px");
	}
	if (topval < 110) {
		topval = 110;
	}
	...
	if (widthval + leftval > 1190) {
		leftval = 1190 - widthval;
	}
	if (heightval + topval > 650) {
		topval = 650 - heightval;
	}
	$("#distop").val(topval);
	$("#disheight").val(heightval);
	$("#disleft").val(leftval);
	$("#diswidth").val(widthval);
	$("#replace").css("top", topval + "px");
	$("#replace").css("height", heightval + "px");
	$("#replace .dialogbody").css("height", heightval);
	$("#replace").css("width", widthval + "px");
	$("#replace").css("left", leftval + "px");
}
